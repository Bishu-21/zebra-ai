...
export function compileTailoredResumeContent(
    baseResumeContent: string, 
    approvedChanges: Array<{ section: string; changeType: string; originalText: string | null; suggestedText: string; userEdits: string | null }>
): string {
    let isJson = false;
    ...
    if (!isJson) {
        // Safe compilation for raw text and LaTeX resumes without destroying original text
        ...
        for (const change of approvedChanges) {
            ...
            if (originalText && updatedText.includes(originalText)) {
                if (changeType === "remove") {
                    updatedText = updatedText.replace(originalText, "");
                } else {
                    updatedText = updatedText.replace(originalText, appliedText);
                }
    ...
    if (!parsed.basics) {
        parsed.basics = { name: "", email: "", phone: "", summary: "", location: "" };
    }
    ...
    for (const change of approvedChanges) {
        ...
        if (section.includes("summary")) {
            if (changeType === "remove") {
                ...
                parsed.basics.summary = appliedText;
            }
        } else if (section.includes("experience")) {
            if (!Array.isArray(parsed.experience)) parsed.experience = [];
            if (parsed.experience.length === 0) {
                if (changeType !== "remove") {
                    parsed.experience.push({ id: 1, role: "Relevant Role", company: "Company", period: "Present", highlights: [appliedText] });
                }
            } else {
                let replaced = false;
                if (originalText) {
                    for (const exp of parsed.experience) {
                        if (Array.isArray(exp.highlights)) {
                            // Exact string matching to prevent false positive substring replacements
                            const idx = exp.highlights.findIndex((h: string) => typeof h === "string" && (h.trim() === originalText || h === originalText));
                            if (idx !== -1) {
                                if (changeType === "remove") {
                                    exp.highlights.splice(idx, 1);
                                } else {
                                    exp.highlights[idx] = appliedText;
                                }
                                replaced = true;
                                break;
                            }
                ...
                if (!replaced && changeType !== "remove") {
                    ...
                    firstExp.highlights = firstExp.highlights || [];
                    // Idempotency: prevent duplicate addition of identical highlight
                    if (!firstExp.highlights.includes(appliedText)) {
                        firstExp.highlights.push(appliedText);
                    }
                }
            }
            ...
            if (parsed.skills.length === 0) {
                if (changeType !== "remove") {
                    parsed.skills.push({ id: 1, category: "Technical Skills", items: appliedText });
                }
            } else {
                const existingItems = parsed.skills[0].items || "";
                if (changeType === "remove") {
                    const itemList = existingItems.split(",").map(i => i.trim());
                    parsed.skills[0].items = itemList.filter(i => i !== originalText && i !== appliedText).join(", ");
                    ...
                    const itemList = existingItems.split(",").map(i => i.trim());
                    if (!itemList.includes(appliedText)) {
            ...
            if (parsed.projects.length === 0) {
                if (changeType !== "remove") {
                    parsed.projects.push({ id: 1, title: "Featured Project", techStack: "", highlights: [appliedText] });
                }
            } else {
                let replaced = false;
                if (originalText) {
                    for (const proj of parsed.projects) {
                        if (Array.isArray(proj.highlights)) {
                            // Exact string matching for project highlights
                            const idx = proj.highlights.findIndex((h: string) => typeof h === "string" && (h.trim() === originalText || h === originalText));
                            if (idx !== -1) {
                                if (changeType === "remove") {
                                    proj.highlights.splice(idx, 1);
                                } else {
                                    proj.highlights[idx] = appliedText;
                                }
                                replaced = true;
                                break;
                            }
                ...
                if (!replaced && changeType !== "remove") {
                    ...
                    firstProj.highlights = firstProj.highlights || [];
                    // Idempotency: prevent duplicate addition
                    if (!firstProj.highlights.includes(appliedText)) {
                        firstProj.highlights.push(appliedText);
                    }
                }
            }
        }
