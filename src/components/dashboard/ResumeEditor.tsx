            ...
            <header className="h-11 bg-background border-b border-border-subtle flex items-center justify-between px-3 shrink-0 select-none relative z-[150]">
                <div className="flex items-center gap-3">
                    ...
                    <div className="hidden sm:block">
                        <span className="px-2 py-0.5 bg-primary/10 text-primary rounded-[var(--radius-sm)] text-[10px] font-semibold tracking-wide">DRAFT</span>
                    </div>
                </div>
...
function ProjectsEditor({ content, updateProject, addProject, removeItem, getAiSuggestions }: { content: ResumeContent; updateProject: (id: number, f: string, v: string | string[]) => void; addProject: () => void; removeItem: (s: 'projects', id: number) => void; getAiSuggestions: (s: string, t: string, c?: { id: number; idx: number }) => void }) {
    return (
        <div className="space-y-4">
...
function ExperienceEditor({ content, updateExperience, addExperience, removeItem, getAiSuggestions }: { content: ResumeContent; updateExperience: (id: number, f: string, v: string | string[]) => void; addExperience: () => void; removeItem: (s: 'experience', id: number) => void; getAiSuggestions: (s: string, t: string, c?: { id: number; idx: number }) => void }) {
    return (
        <div className="space-y-4">
